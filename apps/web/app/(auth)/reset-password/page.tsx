...
function ResetPasswordForm() {
  ...
  if (isVerifying) {
    return (
      <div className="flex flex-col items-center justify-center space-y-4 py-12 text-center">
        <Loader2
          size={36}
          className="animate-spin text-violet-600 dark:text-violet-500"
        />
  ...
  if (!isValidToken && !isSuccess) {
    return (
      <div className="space-y-6">
        <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-red-500/20 bg-red-50 text-red-600 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-400">
          <ShieldAlert size={28} />
        </div>
        ...
        <div className="space-y-3 pt-2">
          ...
          <Link
            href="/signin"
            className="inline-flex h-11 w-full items-center justify-center rounded-xl border border-zinc-200 bg-white text-sm font-semibold text-zinc-800 shadow-xs transition-all hover:bg-zinc-50 active:scale-[0.98] dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:bg-white/10"
          >
            Back to sign in
          </Link>
        </div>
  ...
  if (isSuccess) {
    return (
      <div className="space-y-6">
        <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-emerald-500/20 bg-emerald-50 text-emerald-600 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-400">
        ...
        <div className="pt-2">
          <Link
            href="/signin"
            className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-violet-600 text-sm font-semibold text-white shadow-xs transition-all hover:bg-violet-700 active:scale-[0.98]"
          >
        ...
        <div className="group/input space-y-2">
          ...
          >
            ...
            <div className="flex gap-4 text-[11px] font-medium">
              <span
                className={`flex items-center gap-1 transition-colors ${hasMinLength ? "text-emerald-600 dark:text-emerald-400" : "text-zinc-400 dark:text-zinc-500"}`}
              >
                {hasMinLength ? (
              ...
              <span
                className={`flex items-center gap-1 transition-colors ${hasSymbol ? "text-emerald-600 dark:text-emerald-400" : "text-zinc-400 dark:text-zinc-500"}`}
              >
                {hasSymbol ? (
                  <CheckCircle2 size={12} />
                ) : (
